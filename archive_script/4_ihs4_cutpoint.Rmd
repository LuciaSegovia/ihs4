...
```{r}
library(viridis) #cool colors for figures using color gradients
library(ggpubr) #multiple plot panels in same figure
library(epiDisplay)
library(plyr)
library(magrittr)
library(foreign)
...
```{r}
 ihs4 <- read.csv(here::here("data", "inter-output", "hh_mod_g_kg.csv"))
...
Consumption quantity generally tends to be non-normally distributed with a right skew, so to define a more approriate cut-off, we can just log-transform the distribution to normalize and then take +2SDs from the mean to identify households with unreasonable consumption estimates. The households which lie in the upper limit tail, we can cap their total consumption of the food item at whatever consumption quantity the +4/5SD from mean represents 
```{r}
hh_roster <- read.csvhere::here("data",  "hh_mod_b.csv")
...
#Consumption (kg/capita/day) (114= spaghetti, 311=groundnuts, 408= tomatos, 101= maize flour,)
ihs4 %>% filter(item_code==407) %>% ggplot(., aes(x=kg_pc_d)) + 
...
ihs4 %>% filter(item_code==101) %>% 
    ggplot(aes(sample=kg_pc_d)) + 
    stat_qq() +
    stat_qq_line()


#Consumption (log(kg/capita/day))
ihs4$logkg <- log(ihs4$kg_pc_d)

ihs4 %>% filter(item_code==311) %>% ggplot(., aes(x=logkg)) + 
    geom_histogram(aes(y=..density..),
                   colour="black", fill="white") +
    theme_bw() +
    geom_density(alpha=.2, fill="#FF6666")
...
ihs4 %>% filter(item_code==408) %>% 
    ggplot(aes(sample=logkg)) + 
    stat_qq() +
    stat_qq_line()
...
```{r}
fctmatch <- read.csv(here::here("data", "hs4_fctmatch.csv"))

mwifct <- read.csv(here::here("data",  "mwi_fct.csv"))
...
test <- fcode_match %>% select(code, item, item_eng, yield.assumption)

# write.csv(test, here::here("data", "inter-output", "items_match.csv")
...
```{r}
ihs4 %>% filter(!is.na(cons_unitA)) %>% select(cons_unit, cons_unitA)
