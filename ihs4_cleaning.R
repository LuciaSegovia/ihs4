...
# Using food consumed in kg/d calculated from total consumption (same quantity & same units) to own production
ihs4  <- ihs4 %>% 
...
# Using food consumed in kg/d calculated from total consumption (same quantity, but different units) to own production - Assumption.         
ihs4  <- ihs4 %>%         
  mutate(
...
# Calculating from factor from NSU from consumed to own production
...
ihs4 %>% 
  ...
  filter(prod_quant >0) %>% select(ends_with("quant"), cons_unitA, prod_unitA, item_code_st, region) %>% 
  left_join(., nsu.df, by = c("item_code_st"= "item_code", "region", "prod_unitA" = "cons_unitA"))
