...
ame <- read.csv(here::here("data", "ame.factors.csv"))

ame.spec <- read.csv(here::here("data", "ame.spec.csv"))
names(ame.spec)
...
ihs4.roster.c <- ihs4.roster.c %>% mutate(age.m.total = ifelse(is.na(age_y), age_m, 
                                              ifelse(is.na(age_m),  age_y*12, 
                                            age_y*12 + age_m)))
...
write.csv(hme, here::here("data", "inter-output", "ihs4.afe_v1.0.0.csv"), row.names = FALSE)
